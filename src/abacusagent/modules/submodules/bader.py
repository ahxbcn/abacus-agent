...
def calculate_bader_charges(
    ...
    cube_filename = fcube.name
    local_cube_path = current_dir / cube_filename

    if not local_cube_path.exists():
        local_cube_path.symlink_to(fcube)
        print(f"Created symbolic link: {local_cube_path} -> {fcube}")
    
    cmd = f'{BADER_EXE} {local_cube_path}'
    print(f"Running command: {cmd}")
    return_code, out, err = run_command(cmd, shell=True)
    os.unlink(local_cube_path)  # Remove the symbolic link after use
    if return_code != 0:
